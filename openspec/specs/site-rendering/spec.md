...
Renders the parsed and enriched guide data as a mobile-first static Astro site — a season index plus one page per season board — with design-token styling, category navigation, show cards, and a quick-stats footer.

## Requirements

### Requirement: Render mobile-first static site from enriched guide
The system SHALL render a static HTML site from the parsed + enriched guide data using Astro. The site SHALL be mobile-first: single-column layout on viewports < 640px, expanding to 2 columns at ≥ 640px and 3 columns at ≥ 1024px. The site SHALL consist of a season index page at the root plus one season page per board under `boards/`, each season page rendering all of that board's categories. All styling SHALL consume the centralized design tokens (`var(--*)` from `src/styles/tokens.css`); dark mode SHALL be the default theme.
